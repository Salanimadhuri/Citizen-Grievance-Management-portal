...
const CitizenCommunication = () => {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      // only show assigned complaints (have an officer)
      setComplaints(Array.isArray(data) ? data.filter(c => c.assignedOfficer || c.officerId) : []);
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    const officer = selectedComplaint.assignedOfficer || selectedComplaint.officerId;
    const receiverId = officer?.id || officer?.id || officer;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Message officers about your complaints</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Complaints List */}
        <div className="card">
          <h3 className="font-semibold text-gray-900 mb-4">Your Complaints</h3>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {complaints.length > 0 ? complaints.map((complaint) => {
              const id = complaint.id || complaint._id;
              const officer = complaint.assignedOfficer || complaint.officerId;
              return (
                <button
                  key={id}
                ...
                >
                  ...
                  <p className="text-sm text-gray-600">Officer: {officer?.name || 'Not assigned'}</p>
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">No assigned complaints yet</p>
              </div>
            )}
          </div>
        </div>
            ...
            <>
              <div className="border-b border-gray-200 pb-4 mb-4">
                ...
                <p className="text-sm text-gray-600">
                  Officer: {(selectedComplaint.assignedOfficer || selectedComplaint.officerId)?.name || 'Not assigned'}
                </p>
              </div>
