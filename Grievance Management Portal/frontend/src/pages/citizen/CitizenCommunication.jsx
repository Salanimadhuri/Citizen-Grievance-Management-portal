  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await communicationAPI.sendMessage({
        ...
        receiverId: selectedComplaint.assignedOfficer?._id,
        message: newMessage,
      });
      setNewMessage('');
      fetchMessages(selectedComplaint._id);
            ...
            {complaints.length > 0 ? (
              complaints.map((complaint) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600">Officer: {complaint.assignedOfficer?.name || 'Not assigned'}</p>
          ...
          {selectedComplaint ? (
            <>
              <div className="border-b border-gray-200 pb-4 mb-4">
                ...
                <p className="text-sm text-gray-600">Officer: {selectedComplaint.assignedOfficer?.name || 'Not assigned'}</p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto mb-4 space-y-3 max-h-96">
                {messages.length > 0 ? (
                  messages.map((msg) => (
                    <div
                      key={msg._id}
                      className={`flex ${msg.senderId._id === msg.senderId._id ? 'justify-end' : 'justify-start'}`}
                    >
                      <div
                        className={`max-w-xs px-4 py-2 rounded-lg ${
                          msg.senderId.role === 'citizen'
                            ? 'bg-primary-100 text-gray-900'
                            : 'bg-gray-100 text-gray-900'
                        }`}
                      >
                        <p className="text-sm font-medium">{msg.senderId.name}</p>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={sending || !newMessage.trim() || !selectedComplaint.assignedOfficer}
