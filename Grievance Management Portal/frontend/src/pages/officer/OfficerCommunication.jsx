  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await communicationAPI.sendMessage({
        ...
        receiverId: selectedComplaint.userId._id,
        message: newMessage,
      });
      setNewMessage('');
      fetchMessages(selectedComplaint._id);
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {conversations.length > 0 ? (
              conversations.map((conv) => (
                <button
                  key={conv._id}
                  onClick={() => setSelectedComplaint(conv.complaint)}
                  className={`w-full text-left p-3 rounded-lg transition-colors ${
                    selectedComplaint?._id === conv._id
                      ? 'bg-primary-100 border border-primary-300'
                      : 'hover:bg-gray-100 border border-gray-200'
                  }`}
                >
                  <p className="font-medium text-gray-900 truncate">{conv.complaint.title}</p>
          ...
          {selectedComplaint ? (
            <>
              <div className="border-b border-gray-200 pb-4 mb-4">
                ...
                <p className="text-sm text-gray-600">Citizen: {selectedComplaint.userId?.name}</p>
              </div>
